...
void Controller::callForHeat(bool value)
{
    if (value) {
        ...
        qDebug() << qPrintable(QString("%1 END heat; temp = %2 (%3)")
                               .arg(QDateTime::currentDateTime().toString())
                               .arg(currentTemperature)
                               .arg(QDateTime::currentSecsSinceEpoch() - callForHeatTime.toSecsSinceEpoch()));
...
{
    callForHeat(false);
    qDebug() << qPrintable(QString("%1 FAULT - temp receive timeout").arg(QDateTime::currentSecsSinceEpoch()));
}
